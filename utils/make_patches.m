...
files=dir('*_masks.png'); % we only use images for which we have a mask available, so we use their filenames to limit the patients
...
for ci=1:length(patients) % for each of the *patients* we extract patches
    ...
    files=dir(sprintf('%s_*.png',patients{ci})); %get a list of all of the image files associated with this particular patient
    ...
    for fi=1:length(files) %for each of the files.....
        ...
        fname_mask=strrep(fname,'_original.tif','_masks.png'); %and the mask name
